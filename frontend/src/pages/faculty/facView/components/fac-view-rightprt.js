...
function FacViewRightprt() {
  ...
  useEffect(() => {
    if (Array.isArray(termStudents)) {
        ...
        const submitted = termStudents.filter((student) => student.status?.toLowerCase() === 'submitted').length;
  ...
  const filteredStudentData = Array.isArray(termStudents)
    ? termStudents.filter((student) => {
        if (activeTab === 'submitted') {
          return student.status?.toLowerCase() === 'submitted';
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest('.dropdown-menu') && !event.target.closest('.downloadBtn')) {
        setDropdownVisible({
          downloadMenu: false,
          download1Menu: false,
          download2Menu: false,
